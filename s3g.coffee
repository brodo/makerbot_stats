  ...
  parse: (parameters, payload) ->
    ...
    for parameter in parameters
      ...
      switch type
        ...
        when "ToolQuery"
          @ToolCommand = s3g_constants.nameForToolByte(payload[0])
...
requestStateMachine = new S3GPacketStateMachine((buffer) -> 
  packet = new RequestPacket(buffer)
  if packet.expectsAnswer() then UnansweredPackageQueue.push(packet)
  delete packet.responseParameters 
  if module.exports.callback then module.exports.callback(packet) else console.log(packet)
  
)
responseStateMachine = new S3GPacketStateMachine((buffer) ->
  # If there is no Unansered request, we don't know what this is the answer to
  if UnansweredPackageQueue.length > 0
    packet = new ResponsePacket(buffer, UnansweredPackageQueue.shift().responseParameters)
    delete packet.responseParameters 
    if module.exports.callback then module.exports.callback(packet) else console.log(packet)
)

module.exports.readRequestByte = requestStateMachine.read
module.exports.readRequestByte = responseStateMachine.read
